...
dir_name <- paste(cluster, "best_trajectories_50migrants/params_2348", sep = "/")
dirs=dir(dir(dir_name, full.names = TRUE), full.names = TRUE, pattern = "perc_0.05|perc_0.3")
...
for(i in 1:length(dirs_illumina)){
  ...
  if(((length(files_shiver)/7) != length(files_illumina)) & (length(files_illumina) != 0)){
    ...
    #remove all IDs that have an equivalent shiver files
    done_files_illumina <- files_illumina[- index]

    for(f in 1:length(done_files_illumina)){
      command_rm_rest <- paste("rm -rf", done_files_illumina[f])
      system(command_rm_rest)
    }
