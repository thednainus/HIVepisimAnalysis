...
W_threshold <- W1[W1$infectorProbability >= threshold,]

tipsToAnalyse <- unique(c(W_threshold$donor, W_threshold$recip))
...
tree_name <- vts_trees_years[1]

filename_prefix <- str_split(tree_name, pattern = "\\.")[[1]][1]
filename_prefix <-str_split(filename_prefix, pattern = "/")[[1]][4]

if (filename_prefix == "results_sampling_consensus") {
  if (!dir.exists("output_deepseq/vts/merged_trees/consensus_tree")) {
    dir.create("output_deepseq/vts/merged_trees/consensus_tree")
  }

  input_seqgen <-  paste("output_deepseq/vts/merged_trees/consensus_tree",
                         "tmp_inputSeqGen_consensus.txt",
                          sep = "/")
...
IDs_to_keep <- str_split(tipsToAnalyse, pattern = "_")
IDs_to_keep <- unlist(lapply(IDs_to_keep, function(x) x[1]))

all_IDs <- grepl(pattern = paste(IDs_to_keep, collapse="|"),
...
if (filename_prefix != "results_sampling_consensus") {
  ...
  #get ID names
  ID_names <-  unlist(lapply(names(genome_seqs), function(x)
                            str_split(string = x, pattern = "_")[[1]][2]))
  ...
  lapply(seq_by_ID, function(x)
    write.FASTA(x,file = paste( dir_name_by_tree, "/",
