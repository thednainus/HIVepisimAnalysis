...
#untar tar file
simulation_data <- "/Users/user/Desktop/Imperial/newHIVproject-01Aug2020/R_projects/Results_paper/best_trajectories_500migrants/params_1067/rep_1/sim_results.tar.gz"
...
parameters <- "-demoModel Logistic -N0 0.00593 -growthRate 2.851904 -t50 -2 -forceCoalescence"
parameters <- "-demoModel Constant -N0 100"
...
st_ids_region_all <- sampleIDs(perc = 0.9,
...
#st_ids_region <- list_samplesToAnalyse[!duplicated(list_samplesToAnalyse[ , "sampled_ID"]), ]
st_ids_region <- list_samplesToAnalyse[list_samplesToAnalyse$sampled_ID == 15279,]
#st_ids_region$sampled_time[2] <- 2009.1
#st_ids_region["date"] <-
...
all_IDs2 <- grepl(pattern = "sampled_1_", x = vts_tree$tip.label)
...
quartz()
plot(tree_years, main = parameters)
axisPhylo()

fit <- mlskygrid(tree_years, model=2,res = NULL)
quartz()
plot(fit, logy = FALSE)

fit2 <- mlskygrid(tree_years, model=2,res = NULL)
quartz()
plot(fit2, logy = FALSE)

fit1 <- mlskygrid(tree_years, model=1,res = NULL)
quartz()
plot(fit1, logy = FALSE)

fit3 <- mlskygrid(tree_years, model=3,res = NULL)
quartz()
plot(fit3, logy = FALSE)


all_IDs1 <- grepl(pattern = "ID_112599", x = tree_years$tip.label)
all_IDs1 <- setNames(all_IDs1, tree_years$tip.label)
toDrop3 <- all_IDs1[all_IDs1 == FALSE]
#to run the infector probability analysis
tree_years1 <- drop.tip(tree_years, names(toDrop3))
...
fit2 <- mlskygrid(tree_years1, model=3, res = NULL)
plot( fit2, logy=FALSE)


test_treeswift <- read.tree("forceCoa_treeswift.tre")

fit <- mlskygrid(test_treeswift,model=3,res = 40)
plot( fit, logy=FALSE)
