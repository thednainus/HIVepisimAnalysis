...
#setwd("/Users/user/Desktop/Imperial/newHIVproject-01Aug2020/R_projects/HIVepisim/Analyses/Preliminary_results/500pop/results_tergmLite1/run_1/")


#list files
list_files <- list.files("output/vts/alignments", pattern = "_10000bp.fasta", full.names = TRUE)
# list RData that contains sampleTimes
list_sampleTimes <- list.files("output/vts/W", pattern = ".RData", full.names = TRUE)

#make dir to save iqtree
if (!dir.exists("output/vts/alignments/iqtree_results_10000bp")) {
  dir.create("output/vts/alignments/iqtree_results_10000bp")
}
...
for(ali in list_files){
  ...
  # multifurcating.
  iqtree_param <- c(ali_name, "-m HKY+G", "-T AUTO", "-ntmax", maxCPU, "-czb")
  #run iqtree
  ...
  #run treedater
  dated_tree <- dater(tre = mltree, sts = sampleTimes[mltree$tip.label], s = 10000)
  ...
  W_estimated_10000bp <- phylo.source.attribution.hiv.msm( region_only_dated_tree, sampleTimes[region_only_dated_tree$tip.label],
  ...
  save(years, MH, max_value, last_sample_date, tm, tm2, region_only_dated_tree,
       sampleTimes, all_cd4s, ehis, newinf_per_year, totalPLWHIV, W_estimated_10000bp,
       file = W_filename)
}
