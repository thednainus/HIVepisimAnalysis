...
quartz()
testt %>%
  group_by(param, perc, FPR, TPR) %>%
  summarize(lquartile = quantile(TPR, prob=0.25),
            uquartile = quantile(TPR, prob=0.75),
            sensitivity = median(TPR),
            .groups="drop") %>%
  ggplot(aes(x=FPR, y=TPR,
             ymin=lquartile, ymax=uquartile)) +
  facet_wrap(~ perc, scales = "free", ncol = 4) +
  geom_abline(slope= 1, intercept= 0, linetype = 4) +
  geom_ribbon(alpha=0.25, aes(fill=param)) +
  geom_step(aes(color=param)) +
  theme_bw() +
...
roc_true_trees_s1_250mig <- readRDS("roc_data_250mig.RDS")
#convert list to dataframe
roc_true_trees_s1_250mig_df <- do.call(rbind, roc_true_trees_s1_250mig)
all_true_roc_s1_250mig_noNA <- na.omit(roc_true_trees_s1_250mig_df)
...
teste_roc <- subset(teste, perc == "0.05" | perc == "0.9")
teste_roc_rep1 <- subset(teste, (perc == "0.05" | perc == "0.9") & rep == "1")
teste_roc_0.9 <- subset(teste, perc == "0.9")
quartz()
ggplot(data=roc_true_trees_s1_250mig_df, aes(x=FPR, y=TPR, colour = param)) +
  geom_line() +
...
ggplot(data=teste_roc, aes(x=FPR, y=TPR, colour = param)) +
  geom_line() +
  geom_abline(slope= 1, intercept= 0, linetype = 4) + theme_bw(base_size = 20) +
  facet_wrap(~ perc, scales = "free", ncol = 4)+
  theme(legend.position="bottom") +
