...
#true trees
true_tree_s1 <- readRDS("~/Box Sync/HIV_SanDiego/data_simulations/ROC_data_myScript/by_replicate/roc_trueTrees_s1_CI.RDS")
true_tree_s1 <- do.call(rbind, true_tree_s1)
true_tree_s2 <- readRDS("~/Box Sync/HIV_SanDiego/data_simulations/ROC_data_myScript/by_replicate/roc_trueTrees_s2_CI.RDS")
true_tree_s2 <- do.call(rbind, true_tree_s2)

#ML tree: 1000bp
ml1000bp_s1 <- readRDS("~/Box Sync/HIV_SanDiego/data_simulations/ROC_data_myScript/by_replicate/roc_ML1000bp_s1_CI.RDS")
ml1000bp_s1 <- do.call(rbind, ml1000bp_s1)

ml1000bp_s2 <- readRDS("~/Box Sync/HIV_SanDiego/data_simulations/ROC_data_myScript/by_replicate/roc_ML1000bp_s2_CI.RDS")
ml1000bp_s2 <- do.call(rbind, ml1000bp_s2)

#ML tree: 10000bp
ml10000bp_s1 <- readRDS("~/Box Sync/HIV_SanDiego/data_simulations/ROC_data_myScript/by_replicate/roc_ML10000bp_s1_CI.RDS")
ml10000bp_s1 <- do.call(rbind, ml10000bp_s1)
ml10000bp_s2 <- readRDS("~/Box Sync/HIV_SanDiego/data_simulations/ROC_data_myScript/by_replicate/roc_ML10000bp_s2_CI.RDS")
...
sampler1_500mig <- subset(sampler1, mig == "500")
...
sampler2_500mig <- subset(sampler2, mig == "500")
...
m_and_qt <- function(dataframe){
  ...
  all_qt <- aggregate(dataframe[,2] ~ FPR, data = dataframe,
                      FUN=quantile, probs = c(0.025, 0.5, 0.975))

  all_qt <- data.frame(FPR = all_qt$FPR, all_qt$`dataframe[, 2]`)
...
quartz()
teste <- subset(sampler1_1067_500mig, perc == "0.9" & code == "TrueTrees")
teste["param_mig_code"] <- paste(teste$param, teste$mig, teste$code, sep = "_")

data_plot <- teste %>%
  drop_na() %>%
  group_by(code, sampler, param, perc, mig, FPR, param_mig_code) %>%
  group_modify(~ {
    print(.x$TPR)
    summarize(lquartile = quantile(.x$TPR, prob = 0.25),
              uquartile = quantile(.x$TPR, prob = 0.75),
              TPR = median(.x$TPR),
              .groups="drop") })


ggplot(all_qt, aes(x = FPR, y = X50.)) +
  geom_abline(slope = 1, intercept= 0, linetype = 4) +
  geom_ribbon(alpha=0.25, aes(ymin=X2.5., ymax=X97.5.)) +
  geom_step(size = 0.8) +
  theme_bw()
...
%>%
  ggplot(aes(x = FPR, y = TPR, ymin=lquartile, ymax=uquartile)) +
  geom_ribbon(alpha=0.25, aes(fill = param_mig_code)) +
  geom_abline(slope = 1, intercept= 0, linetype = 4) +
